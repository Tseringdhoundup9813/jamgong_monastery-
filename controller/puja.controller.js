...
exports.CreatePuja = asyncErrorHandler(async(req,res,next)=>{
        ...
        if(req.file){
            ...
            const newPuja = await pujaModel.create({image:filePath,
                                        title,durationAndPrice:durationAndPriceArray,
...
exports.SelectedPujaList = asyncErrorHandler(async(req,res,next)=>{
    ...
    if(checkSponsorExits){
        await pujaSelectModel.updateOne({sponsor:id},{selectedlist:req.body})
    }else{
